...
export default function MuhasebeModal({ jobs, canEditAcc, onClose }) {
  ...
  const th = (right) => ({
    ...
    textAlign: right ? 'right' : 'left', borderBottom: '1px solid #141414',
    position: 'sticky', top: 0, background: '#0c0c0c', whiteSpace: 'nowrap', zIndex: 10
  })
  ...
  const toggleOdeme = async (job) => {
    ...
    if (tamOdendi) {
      ...
      // Tam ödendi — ödenen = ederi
      await updateJob(job.id, { odenen: ederi.toString(), durum: 'kapandi' })
    }
  }

  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: '#0c0c0c', border: '1px solid #1e1e1e', borderRadius: 8, width: '96vw', maxWidth: 1000, height: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
        onClick={e => e.stopPropagation()}>

        {/* Başlık */}
        <div style={{ padding: '14px 22px', borderBottom: '1px solid #161616', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#d97706', fontFamily: "'IBM Plex Sans',sans-serif" }}>₺ Muhasebe</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {/* Sekmeler */}
            <div style={{ display: 'flex', background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: 6, padding: 3, gap: 2 }}>
              <button onClick={() => setSekme('bekleyen')}
                style={{ padding: '5px 16px', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: sekme === 'bekleyen' ? 600 : 400, fontFamily: "'IBM Plex Sans',sans-serif", background: sekme === 'bekleyen' ? '#1e1e1e' : 'transparent', color: sekme === 'bekleyen' ? '#f87171' : '#666', transition: 'all .15s' }}>
                Ödeme Bekleyen
                <span style={{ marginLeft: 6, fontSize: 9, color: sekme === 'bekleyen' ? '#f87171' : '#333' }}>{bekleyen.length}</span>
              </button>
              <button onClick={() => setSekme('odenmis')}
                style={{ padding: '5px 16px', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: sekme === 'odenmis' ? 600 : 400, fontFamily: "'IBM Plex Sans',sans-serif", background: sekme === 'odenmis' ? '#1e1e1e' : 'transparent', color: sekme === 'odenmis' ? '#4ade80' : '#666', transition: 'all .15s' }}>
                Ödemesi Yapılanlar
                <span style={{ marginLeft: 6, fontSize: 9, color: sekme === 'odenmis' ? '#4ade80' : '#333' }}>{odenmis.length}</span>
              </button>
            </div>
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead>
              <tr>
                <th style={{ ...th(false), color: '#888' }}>Sınıf-Kod</th>
                <th style={{ ...th(false), color: '#888' }}>Kategori</th>
                <th style={{ ...th(false), color: '#888' }}>S. Veren</th>
                ...
                <th style={{ ...th(false), color: '#888' }}>Ödeme Tarihi</th>
                <th style={{ ...th(true), color: '#4ade80', width: 60 }}>Ödendi</th>
              </tr>
            </thead>
            <tbody>
              {liste.length === 0
                ? <tr><td colSpan={10} style={{ textAlign: 'center', padding: 48, color: '#333', fontSize: 11 }}>
                ...
                : liste.map(job => {
                  ...
                  return (
                    <tr key={job.id} style={{ opacity: sekme === 'odenmis' ? 0.7 : 1 }}>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' }}>
                        {job.sinifi && job.kodu ? `${job.sinifi}-${job.kodu}` : job.kodu || job.sinifi || '—'}
                      </td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', fontSize: 12, whiteSpace: 'nowrap' }}>{job.kategori || '—'}</td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', fontSize: 12, whiteSpace: 'nowrap' }}>{job.siparisiVeren || '—'}</td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', textAlign: 'right' }}>
                        {canEditAcc
                      ...
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', textAlign: 'right', fontSize: 12, color: '#e0e0e0' }}>{ederi > 0 ? fmt(ederi) : '—'}</td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', textAlign: 'right' }}>
                        {canEditAcc
                      ...
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', textAlign: 'right', fontSize: 12, fontWeight: 500, color: kalan <= 0 ? '#4ade80' : '#f87171' }}>
                        {(ederi > 0 || odenen > 0) ? fmt(kalan) : '—'}
                      </td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111' }}>
                        {canEditAcc
                          ...
                          : <span style={{ fontSize: 12, color: '#888' }}>{job.odemeTarihi || '—'}</span>}
                      </td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #111', textAlign: 'center' }}>
        ...
        {/* Sabit toplam */}
        <div style={{ flexShrink: 0, borderTop: '2px solid #1e1e1e', background: '#0a0a0a', padding: '14px 22px' }}>
          <div style={{ fontSize: 8, color: '#444', letterSpacing: '0.15em', textTransform: 'uppercase', fontFamily: "'IBM Plex Sans',sans-serif", marginBottom: 12 }}>
            {sekme === 'bekleyen' ? 'Ödeme Bekleyenler Toplamı' : 'Ödemesi Yapılanlar Toplamı'}
          </div>
          <div style={{ display: 'flex', gap: 32, alignItems: 'flex-end' }}>
            {[
              { l: 'Toplam Ederi', v: t.ederi, c: '#c0c0c0' },
              { l: 'Toplam Ödenen', v: t.odenen, c: '#4ade80' },
              { l: 'Kalan', v: t.kalan, c: t.kalan > 0 ? '#f87171' : '#4ade80' }
            ].map(({ l, v, c }, i) => (
              <React.Fragment key={l}>
                {i > 0 && <div style={{ width: 1, height: 44, background: '#1e1e1e', alignSelf: 'center' }} />}
                <div>
                  <div style={{ fontSize: 9, color: '#555', letterSpacing: '0.12em', textTransform: 'uppercase', fontFamily: "'IBM Plex Sans',sans-serif", marginBottom: 4 }}>{l}</div>
                  <div style={{ fontSize: 22, fontWeight: 600, color: c, fontFamily: "'IBM Plex Mono',monospace", letterSpacing: '-0.02em' }}>{fmt(v)}</div>
                  <div style={{ fontSize: 9, color: '#333', marginTop: 2 }}>₺</div>
                </div>
              </React.Fragment>
            ))}
          </div>
