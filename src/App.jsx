...
export default function App() {
  ...
  const handleTema = async (yeniTema) => {
    setTema(yeniTema)
    if (session) await updateUserInDB(session.id, { ...session, tema: yeniTema })
  }
  ...
  if (loading) return <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text4)', fontSize: 11, letterSpacing: '0.15em' }}><style>{CSS}</style>Bağlanıyor…</div>
    ...
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 2, background: 'var(--bg5)', border: '1px solid var(--border2)', borderRadius: 5, padding: 2 }}>
            {[['dark','◉','Koyu'],['dim','◑','Loş'],['light','○','Açık']].map(([t, icon, label]) => (
              <button key={t} onClick={() => handleTema(t)} title={label}
                style={{ padding: '3px 9px', borderRadius: 3, border: 'none', cursor: 'pointer', fontSize: 11, background: tema === t ? 'var(--bg6)' : 'transparent', color: tema === t ? '#f59e0b' : 'var(--text4)', transition: 'all .15s' }}>
                {icon}
              </button>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px', background: 'var(--bg5)', borderRadius: 4, border: '1px solid var(--border)' }}>
            <span style={{ fontSize: 12, color: 'var(--text2)', fontWeight: 500 }}>{session.displayName}</span>
          ...
          : <>
            ...
            <div style={{ padding: '14px 20px 10px', background: 'var(--bg)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: tab === 'aktif' ? 8 : 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg5)', border: '1px solid var(--border2)', borderRadius: 7, padding: 3, gap: 2 }}>                  <button onClick={() => setTab(t => t === 'aktif' ? 'all' : 'aktif')}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 5, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 500, fontFamily: "'IBM Plex Sans',sans-serif", background: tab === 'aktif' ? 'rgba(245,158,11,.08)' : 'transparent', color: tab === 'aktif' ? '#f59e0b' : 'var(--text3)', transition: 'all .15s' }}>
                    Aktif <span style={{ fontSize: 9, color: tab === 'aktif' ? 'rgba(245,158,11,.5)' : 'var(--faint)' }}>{cnt('aktif')}</span>
                    <span style={{ fontSize: 8, color: tab === 'aktif' ? '#f59e0b' : 'var(--faint)', marginLeft: 1 }}>{tab === 'aktif' ? '▾' : '▸'}</span>
                  </button>
                  {[['kapandi', 'Kapandı'], ['all', 'Tümü']].map(([k, l]) => {
                    const sel = tab === k
                    return (
                      <button key={k} onClick={() => { setTab(k); setSubTab('uretimde') }}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 5, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: sel ? 600 : 400, fontFamily: "'IBM Plex Sans',sans-serif", background: sel ? 'var(--bg6)' : 'transparent', color: sel ? (DURUM_CLR[k] || 'var(--text)') : 'var(--text3)', boxShadow: sel ? '0 1px 3px rgba(0,0,0,.4)' : 'none', transition: 'all .15s' }}>
                        {DURUM_CLR[k] && <span style={{ width: 5, height: 5, borderRadius: '50%', background: DURUM_CLR[k], display: 'inline-block', opacity: sel ? 1 : .4 }} />}
                        {l} <span style={{ fontSize: 9, color: sel ? (DURUM_CLR[k] || '#666') : 'var(--faint)' }}>{cnt(k)}</span>
                      </button>
                    )
                  })}
                </div>
                ...
                <div style={{ position: 'relative', marginLeft: 'auto' }}>
                  ...
                  <input
                    ...
                    style={{ background: 'var(--bg5)', border: `1px solid ${arama ? '#f59e0b' : 'var(--border2)'}`, borderRadius: 6, padding: '5px 10px 5px 28px', color: 'var(--text)', fontSize: 11, outline: 'none', width: 220, fontFamily: "'IBM Plex Mono',monospace", transition: 'border-color .15s' }}
                  />
                  {arama && <button onClick={() => setArama('')}
                    style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 14, padding: 0, lineHeight: 1 }}>✕</button>}
                </div>
              </div>
              {tab === 'aktif' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, paddingLeft: 4 }}>
                  <span style={{ fontSize: 9, color: 'var(--text3)', marginRight: 2, fontFamily: "'IBM Plex Sans',sans-serif" }}>└</span>
                  {[['uretimde', 'Üretimde', null], ['beklemede', 'Beklemede', '#c0c0c0'], ['onayda', 'Onayda', '#f59e0b'], ['revizyonda', 'Revizyonda', '#c084fc'], ['tamamlandi', 'Tamamlandı', '#4ade80']].map(([k, l, c]) => {
                    ...
                    return (
                      <button key={k} onClick={() => setSubTab(k)}
                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 11px', borderRadius: 4, border: `1px solid ${sel ? 'var(--border4)' : 'transparent'}`, cursor: 'pointer', fontSize: 10, fontWeight: sel ? 600 : 400, fontFamily: "'IBM Plex Sans',sans-serif", background: sel ? 'var(--bg6)' : 'transparent', color: sel ? (c || 'var(--text)') : 'var(--text3)', boxShadow: sel ? '0 1px 3px rgba(0,0,0,.4)' : 'none', transition: 'all .15s' }}>
                        {c && <span style={{ width: 5, height: 5, borderRadius: '50%', background: c, display: 'inline-block', opacity: sel ? 1 : .4 }} />}
                        {l} <span style={{ fontSize: 9, color: sel ? (c || 'var(--text4)') : 'var(--text3)' }}>{subCnt(k)}</span>
                      </button>
                    )
                  })}
                </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Sipariş Tarihi</th><th>Kodu</th><th>Kategori</th>
                  ...
                  {filtered.length === 0
                    ...
                    : filtered.map((job, i) => (
                      <JobRow key={job.id} job={job} idx={i + 1}
                        canEdit={canEdit} isSuperUser={session?.role === 'super'} showAcc={false} canEditAcc={false}
      ...
      {deleteConfirm && (
        <div className="overlay" onClick={() => setDeleteConfirm(null)}>
          <div style={{ background: 'var(--bg5)', border: '1px solid var(--border2)', borderRadius: 8, padding: 28, maxWidth: 300, width: '100%', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 10, color: 'var(--text4)', marginBottom: 22, fontFamily: "'IBM Plex Sans',sans-serif" }}>Bu işlem geri alınamaz.</div>
