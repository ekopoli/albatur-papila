...
import { getUsers, saveUsers } from './firebase.js'
...
const EMPTY_U = { username: '', password: '', displayName: '', role: 'limited' }
...
export function UsersPanel({ session }) {
  ...
  const openEdit = (u) => { setForm({ username: u.username, password: u.password, displayName: u.displayName, role: u.role }); setEditing(u) }
  ...
  const save = () => {
    ...
    if (editing === 'new') persist([...users, { ...form, id: uid() }])
    else persist(users.map(u => u.id === editing.id ? { ...u, ...form } : u))
    closeEdit()
  }

  const del = (id) => { persist(users.filter(u => u.id !== id)); setDelConfirm(null) }
    ...
    <div style={{ maxWidth: 780 }}>
      ...
      <div style={{ background: '#0b0b0b', border: '1px solid #141414', borderRadius: 6, overflow: 'hidden' }}>
        <table>
          <thead>
            <tr>
              <th>Ad Soyad</th><th>Kullanıcı Adı</th><th>Şifre</th><th>Yetki</th>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} style={{ color: '#c0c0c0' }}>
                ...
                <td><span className={`badge b${u.role[0]}`}>{ROLLER[u.role]}</span></td>
          ...
          <div className="mbox" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 22 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13, marginBottom: 14 }}>
                ...
                <div><label className="flbl">Şifre</label><input className="inp" type={isSuper ? 'text' : 'password'} value={form.password} onChange={e => setF('password', e.target.value)} /></div>
                <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, paddingTop: 14, borderTop: '1px solid #111' }}>
                ...
                <button className="btn bA" onClick={save}>{editing === 'new' ? '+ Ekle' : '✓ Kaydet'}</button>
              </div>
            </div>
          </div>
        </div>
