...
export default function App() {
  ...
  const filtered = jobs.filter(j => {
    ...
    // Tüm sekmeler: sınıf-kod küçükten büyüğe
  ...
  if (loading) return <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text5)', fontSize: 11, letterSpacing: '0.15em' }}><style>{CSS}</style>Bağlanıyor…</div>
    ...
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: tab === 'aktif' ? 8 : 0 }}>
                ...
                <div style={{ position: 'relative', marginLeft: 'auto' }}>
                  ...
                  {arama && <button onClick={() => setArama('')}
                    style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--text4)', cursor: 'pointer', fontSize: 14, padding: 0, lineHeight: 1 }}>✕</button>}
                </div>
              </div>
              {tab === 'aktif' && (
      ...
      {deleteConfirm && (
        <div className="overlay" onClick={() => setDeleteConfirm(null)}>
          <div style={{ background: 'var(--bg5)', border: '1px solid var(--border2)', borderRadius: 8, padding: 28, maxWidth: 300, width: '100%', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 22, fontFamily: "'IBM Plex Sans',sans-serif" }}>Bu işlem geri alınamaz.</div>
      ...
      {mailAlert && (
        <div style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', background: 'var(--bg5)', border: '1px solid var(--border2)', borderRadius: 8, padding: '12px 16px', zIndex: 200, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 8px 24px rgba(0,0,0,.6)', maxWidth: 340, width: '90%' }}>
          ...
          <button onClick={() => setMailAlert(null)} style={{ background: 'none', border: 'none', color: 'var(--text5)', cursor: 'pointer', fontSize: 14 }}>✕</button>
        </div>
      )}
    </div>
  )
