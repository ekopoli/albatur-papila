...
import { listenJobs, addJob, updateJob, deleteJob,
         ...
         listenUsers, saveUserToDB, updateUserInDB, deleteUserFromDB, initUsersIfEmpty } from './firebase.js'
...
export default function App() {
  ...
  const filtered = jobs.filter(j => {
    ...
    // aktif tab
    if (subTab === 'all') return isAktif(d)          // Üretimde = tüm aktif (beklemede dahil)
    return d === subTab
  })
  ...
  const subCnt = (s) => {
    if (s === 'all') return jobs.filter(j => isAktif(j.durum || 'beklemede')).length  // tüm aktif
    ...
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: '#080808', minHeight: '100vh', color: '#e0e0e0' }}>
      ...
      <div style={{ padding: '18px 20px' }}>
        {page === 'users' && session.role === 'super'
          ? <UsersPanel users={users} session={session} />
          : <>
            {/* Controls */}
            ...
            {showAccPanel && <div style={{ marginTop: 8, fontSize: 9, color: '#2e2e2e', letterSpacing: '0.1em' }}>* Genel toplam yalnızca <span style={{ color: '#4ade80' }}>Tamamlandı</span> ve <span style={{ color: '#38bdf8' }}>Kapandı</span> durumundaki işleri kapsar.</div>}
          </>
        }
      </div>
