...
export default function App() {
  ...
  // Muhasebe modalını Firebase sync sonrası açmak için ref
  const pendingMuhasebeAc = useRef(null)
  ...
  // jobs güncellendiğinde bekleyen Muhasebe açılışını kontrol et
  useEffect(() => {
    if (!pendingMuhasebeAc.current || !session) return
    const job = jobs.find(j => j.id === pendingMuhasebeAc.current)
    if (job?.durum === 'tamamlandi') {
      const isCanAcc = session.role === 'super' || session.role === 'accview'
      if (isCanAcc) setShowMuhasebe(true)
      pendingMuhasebeAc.current = null
    }
  }, [jobs, session])
  ...
  const handleUpdate = async (id, changes) => {
    ...
    // Tamamlandı → jobs güncellenince useEffect Muhasebe'yi açacak
    if (yeniDurum === 'tamamlandi' && eskiDurum !== 'tamamlandi') {
      pendingMuhasebeAc.current = id
    }
  }
  ...
  const temaCSS = Object.entries(TEMALAR[tema] || TEMALAR.dark).map(([k, v]) => `${k}:${v}`).join(';')

  if (!session) return <><style>{`:root{${temaCSS}}`}{CSS}</style><Login users={users} onLogin={handleLogin} /></>
  if (loading) return <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text3)', fontSize: 11, letterSpacing: '0.15em' }}><style>{`:root{${temaCSS}}`}{CSS}</style>Bağlanıyor…</div>

  const DURUM_CLR = DURUM_RENK

  return (
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)' }}>
      <style>{`:root{${temaCSS}}`}{CSS}</style>
      ...
      <header style={{ background: '#0a0a0a', borderBottom: '1px solid #141414', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 50, position: 'sticky', top: 0, zIndex: 40 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 2, background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: 5, padding: 2 }}>
            {[['dark','◉'],['dim','◑'],['light','○']].map(([t, icon]) => (
              <button key={t} onClick={() => handleTema(t)}
                style={{ padding: '3px 8px', borderRadius: 3, border: 'none', cursor: 'pointer', fontSize: 11, background: tema === t ? '#1e1e1e' : 'transparent', color: tema === t ? '#f59e0b' : '#555', transition: 'all .15s' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 50px)' }}>
        {page === 'users' && session.role === 'super'
          ...
          : <>
            {/* Controls — sabit */}
            <div style={{ padding: '14px 20px 10px', background: '#080808', borderBottom: '1px solid #141414', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: tab === 'aktif' ? 8 : 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: 7, padding: 3, gap: 2 }}>                  <button onClick={() => setTab(t => t === 'aktif' ? 'all' : 'aktif')}
                ...
                <div style={{ position: 'relative', marginLeft: 'auto' }}>
                  ...
                  <input
                    ...
                    style={{ background: '#0e0e0e', border: `1px solid ${arama ? '#f59e0b' : '#1a1a1a'}`, borderRadius: 6, padding: '5px 10px 5px 28px', color: '#e0e0e0', fontSize: 11, outline: 'none', width: 220, fontFamily: "'IBM Plex Mono',monospace", transition: 'border-color .15s' }}
                  />
            ...
            <div style={{ background: '#0b0b0b', borderTop: 'none', minHeight: '100%' }}>
              <table>
                <thead>
                  <tr>
