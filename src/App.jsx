...
export default function App() {
  ...
  // CSS değişkenlerini document'e uygula
  useEffect(() => {
          ...
          <div style={{ display: 'flex', gap: 2, background: '#0e0e0e', border: '1px solid #1a1a1a', borderRadius: 5, padding: 2 }}>
            {[['dark','◉','Koyu'],['dim','◑','Loş'],['light','○','Açık']].map(([t, icon, label]) => (
              <button key={t} onClick={() => handleTema(t)} title={label}
                style={{ padding: '3px 9px', borderRadius: 3, border: 'none', cursor: 'pointer', fontSize: 11, background: tema === t ? '#1e1e1e' : 'transparent', color: tema === t ? '#f59e0b' : '#555', transition: 'all .15s' }}>
                {icon}
              </button>
            ))}
          </div>
