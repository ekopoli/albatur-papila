...
const EMAILJS_SERVICE  = 'service_slel8rg'
const EMAILJS_TEMPLATE = 'template_bvyl1qe'
const EMAILJS_PUBLIC   = 'vFzf9dPBuudScrCmO'
...
export default function App() {
  ...
  const triggerMail = async (job, type = 'new') => {
    const types = {
      new:       { emoji: '📋', subj: `Yeni Sipariş: ${job.kodu || ''} ${job.kategori || ''}` },
      revizyon:  { emoji: '↩',  subj: `Revizyon: ${job.kodu || ''} ${job.kategori || ''}` },
      onaylandi: { emoji: '✓',  subj: `Onaylandı: ${job.kodu || ''} ${job.kategori || ''}` },
    }
    const { emoji, subj } = types[type] || types.new
    ...
    setMailAlert({ label: `${emoji} Gönderiliyor… ${subj}`, status: 'sending' })

    try {
      await emailjs.send(
        EMAILJS_SERVICE,
        EMAILJS_TEMPLATE,
        { to_email: 'ipapila@gmail.com', subject: `ALBATUR-Papila — ${subj}`, message: body },
        EMAILJS_PUBLIC
      )
      setMailAlert({ label: `${emoji} Mail gönderildi: ${subj}`, status: 'ok' })
    } catch (err) {
      console.error('EmailJS hatası:', err)
      setMailAlert({ label: `⚠ Mail gönderilemedi: ${err?.text || err?.message || 'Hata'}`, status: 'err' })
    }
  }
  ...
  const handleUpdate = async (id, changes) => {
    ...
    if (yeniDurum === 'tamamlandi' && eskiDurum !== 'tamamlandi') {
      triggerMail({ ...job, ...changes }, 'onaylandi')
      const kullanicininCanacc = session?.role === 'super' || session?.role === 'accview'
      if (kullanicininCanacc) setShowMuhasebe(true)
    }
  }
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)' }}>
      ...
      {/* Mail alert */}
      {mailAlert && (
        <div style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', borderRadius: 8, padding: '12px 16px', zIndex: 200, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 8px 24px rgba(0,0,0,.6)', maxWidth: 380, width: '90%',
          background: mailAlert.status === 'ok' ? 'rgba(74,222,128,.12)' : mailAlert.status === 'err' ? 'rgba(248,113,113,.12)' : 'var(--bg5)',
          border: `1px solid ${mailAlert.status === 'ok' ? 'rgba(74,222,128,.35)' : mailAlert.status === 'err' ? 'rgba(248,113,113,.35)' : 'var(--border4)'}` }}>
          <span style={{ fontSize: 11,
            color: mailAlert.status === 'ok' ? '#4ade80' : mailAlert.status === 'err' ? '#f87171' : '#aaa',
            flex: 1, fontFamily: "'IBM Plex Sans',sans-serif" }}>{mailAlert.label}</span>
          {mailAlert.status !== 'sending' && (
            <button onClick={() => setMailAlert(null)} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: 14 }}>✕</button>
          )}
        </div>
      )}
    </div>
